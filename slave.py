...
class task_manager:
    ...
    def __init__(self, capacity, mq_name):
        ...
        self.mq = rabbitmq.rabbitmq("172.16.26.111", 5672, "slave", "atislave", mq_name)
        self.mq.connect()
    ...
    def receive_tasks(self):
        while len(self.tasks) < self.capacity:
            ...
            # fresh message
            if not message:
                raw_message = self.mq.receive_message()
                if raw_message:
                    try:
                        message = json.loads(raw_message)
                    except Exception, e:
                        console.log.warning("%r %r" % (e, raw_message))
            if not message:
                break
    ...
    def run_task(self, task):
        ...
        if not success:
            ...
            state = "fail"
            pass
        taskinfo = "name=%s&imp=%d&clk=%d&slave=%s&state=%s" % (str(task["name"]), 1 if str(task["imp"]) else 0, 1 if str(task["clk"]) else 0, self.uuid, state)
        track_task(taskinfo)
